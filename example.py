...
def system_workflow(path_exp, path, architecture, pods, mapping, model_limit, nuser_start=2, ths_filtered=False,
                    generation_conf_FAST=False):
...
def muBench_workflow(path_work, ths_filtered=False, generation_conf_FAST=False):
    path_system = "mubench"
    ...
    system_workflow(path_exp, path_work, utils.get_architecture_from_wm(path_wm), services, {s: s for s in services},
                    model_limit=30, nuser_start=2, ths_filtered=ths_filtered,
                    generation_conf_FAST=generation_conf_FAST)


def sockshop_workflow(path_work, ths_filtered=False, generation_conf_FAST=False):
    ...
    with open(path_arch, 'r') as f_arch:
        with open(path_pods, 'r') as f_pods:
            ...
            with open(path_mapping, "r") as f_map:
                system_workflow(path_exp, path_work, json.load(f_arch), pods, json.load(f_map), model_limit=50,
                                nuser_start=4, ths_filtered=ths_filtered,
                                generation_conf_FAST=generation_conf_FAST)


def trainticket_workflow(path_work, ths_filtered=False, generation_conf_FAST=False):
    ...
    with open(path_arch, 'r') as f_arch:
        with open(path_pods, 'r') as f_pods:
            pods = [p.replace("\n", "") for p in f_pods.readlines()]
            with open(path_mapping, "r") as f_map:
                system_workflow(path_exp, path_work, json.load(f_arch), pods, json.load(f_map), model_limit=30,
                                nuser_start=4, ths_filtered=ths_filtered,
                                generation_conf_FAST=generation_conf_FAST)
